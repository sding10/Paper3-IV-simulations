...
get_first_stage_F <- function(dat, z, x) {
  ...
  if (is.null(an) || !"F" %in% names(an)) {
    return(NA_real_)
  }
  
  F_val <- suppressWarnings(as.numeric(an$F[2]))
...
MRbinary <- function(dat,
  ...
  first_stage_F <- get_first_stage_F(dat, z, x)
