...
## ------------------------------------------------------------
## First-stage model and F diagnostics
## ------------------------------------------------------------
...
MRbinary <- function(dat,
  ...
  F_diag_rep <- F_diag[rep(1, nrow(tab)), , drop = FALSE]
  F_diag_rep <- as.data.frame(F_diag_rep)
  tab <- dplyr::bind_cols(tab, F_diag_rep)
