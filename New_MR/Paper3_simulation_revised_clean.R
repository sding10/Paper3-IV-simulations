...
sim_once <- function(n,
  ...
  if (is.null(fit)) {
    out <- tibble(
      ...
      p = NA_real_,
      conv = 0,
      R2_mcf_2sps = NA_real_,
      R2_cs_2sps = NA_real_,
      R2_nk_2sps = NA_real_,
      F_mcf_2sps = NA_real_,
      F_cs_2sps = NA_real_,
      F_nk_2sps = NA_real_,
      F_anova_2sps = NA_real_
    )
  } else {
...
summarize_sim_results <- function(sim_res) {
  sim_res %>%
    ...
    mutate(
      ...
      type1_pct = 100 * type1,
      power_pct = 100 * power,
      conv_rate = 100 * n_conv / n_sim,
      
      R2_mcf_2sps_text = sprintf(
        "%.2f (%.2f, %.2f)",
        median_R2_mcf_2sps,
        q1_R2_mcf_2sps,
        q3_R2_mcf_2sps
