...
est_ivmvb <- function(dat, z, x, y,
  ...
  if (is.null(start)) {
    ...
    ## First-stage logistic model for X ~ IVs
    form_x <- as.formula(
      paste(x, "~", paste(z, collapse = " + "))
    )
    ...
    a_start <- coef(fit_x_start)
    a_start[!is.finite(a_start)] <- 0
    
    Xmat_start <- model.matrix(form_x, data = dat)
    x_hat_start <- plogis(drop(Xmat_start %*% a_start))
    
    dat$x_hat_start <- x_hat_start
    
    fit_y_start <- glm(
      as.formula(paste(y, "~ x_hat_start")),
      data = dat,
      family = binomial()
    )
    
    b_start <- coef(fit_y_start)
    b_start[!is.finite(b_start)] <- 0
    
    start <- c(a_start, b_start)
  }
